...
class TestProject:
    ...
    def test_project_creation(self):
        ...
        project = Project(name="Test SaaS", variant=Variant.SAAS, budget=10000, timeline_weeks=12)
    ...
    def test_phase_transition(self):
        ...
        # Can transition to next phase (with force=True to bypass quality gates in test)
        success, gate_result = project.transition_to_phase(Phase.IDEATION, force=True)
        assert success is True
        assert project.phase == Phase.IDEATION

        # Cannot skip phases
        success, gate_result = project.transition_to_phase(Phase.EXECUTION, force=True)
